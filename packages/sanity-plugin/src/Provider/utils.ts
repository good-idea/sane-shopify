...
export const testSecrets = async (secrets?: Secrets): Promise<TestResponse> => {
  ...
  if (!storefrontApiKey.length) return { valid: false, message: 'You must provide a Storefront API Key' }
  const response = await createClient(secrets)
    ...
    .catch((e) => ({
      valid: false,
      message: e.message || 'There was an error connecting to Shopify. Check your console for more information.',
    }))
  return response
}
